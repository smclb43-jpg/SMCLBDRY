...
DB_CANDIDATES = [
    ...
    os.path.join(BASE_DIR, "cam_sevkiyat_stok_pro.db"),
]
HOST = "0.0.0.0"
PORT = 8765
...
def find_glass_type(con, tracking_row):
    ...
    for table_name, candidate_columns, fk_name in checks:
        ...
        for col in existing_candidates:
            try:
                lookup_id = int(tracking_row["id"]) if table_name == "production_tracking" else source_id
                row = con.execute(
    ...
    with db_connection() as con:
        ...
        for row in rows:
            jobs.append({
                ...
                "stage": row["stage"],
                "glassType": find_glass_type(con, row),
            })
    return jobs
...
def log_stage_completion(con, row):
    ...
    con.execute(
        ...
        """,
        (
            int(row["id"]), row["item_type"], int(row["source_id"]), row["source_no"],
            row["customer"], row["project"], row["stage"],
            int(row["total_qty"] or 0), float(row["total_m2"] or 0), now, now,
        ),
    )
    ...
    with db_connection() as con:
        for tracking_id in ids:
            ...
            if current_index + 1 < len(route_steps):
                ...
                con.execute(
                    "UPDATE production_tracking SET stage = ?, current_step_index = ?, status = 'in_progress', updated_at = ? WHERE id = ?",
                    (next_stage, current_index + 1, now, int(tracking_id)),
                )
            else:
                con.execute(
                    "UPDATE production_tracking SET status = 'completed', updated_at = ? WHERE id = ?",
                    (now, int(tracking_id)),
                )
            completed += 1
        con.commit()
...
def main():
    print(f"AFTEM Mobile server started: http://127.0.0.1:{PORT}")
    print(f"Use PC IP on the phone. Database: {DB_PATH}")
